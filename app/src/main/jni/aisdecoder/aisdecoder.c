 ...
 *  
 *    Modified 2015 Christian Ebner  <cebner@gmx.at>
 *    based on the source files of rtl_ais from https://github.com/dgiardini/rtl-ais
 *
...
void sound_level_changed(float level, int channel, unsigned char high) {
    if (high != 0)
        aprintf_stderr( "Level on ch %d too high: %.0f %%\n", channel, level);
    else
        aprintf_stderr("Level on ch %d: %.0f %%\n", channel, level);
}
...
void nmea_sentence_received(const char *sentence,
    ...
    if (sentences == 1) {
        ...
        if (debug_nmea) aprintf_stderr("%s", sentence);
    } else {
        ...
        if (sentences == sentencenum && buffer_count > 0) {
            ...
            if (debug_nmea) aprintf(stderr, "%s", buffer);
...
int init_ais_decoder(char * host, char * port ,int show_levels,int _debug_nmea,int buf_len,int time_print_stats){
	...
	aprintf_stderr("init_ais_decoder\n");
	//aprintf(stderr,"Aprintf Test");
	if(debug_nmea)
		aprintf_stderr("Log NMEA sentences to console ON\n");
	else
		aprintf_stderr("Log NMEA sentences to console OFF\n");
...
int initSocket(const char *host, const char *portname) {
    ...
    if (sock==-1) {
        aprintf_stderr( "%s",strerror(errno));
#ifdef WIN32
        WSACleanup();
#endif
        return 0;
    }
	aprintf_stderr("AIS data will be sent to %s port %s\n",host,portname);
    return 1;
}
