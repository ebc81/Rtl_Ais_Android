...
void sample_ready( int16_t* data, int len)
{
	JNIEnv *env;
	if ((*jvm)->GetEnv(jvm, (void **)&env, javaversion) == JNI_EDETACHED)
		(*jvm)->AttachCurrentThread(jvm, &env, 0);
	// write back to Java here
	jshortArray jBuff = (*env)->NewShortArray(env, len);
	if ( jBuff  == NULL )
		return;
	(*env)->SetShortArrayRegion(env,jBuff, 0, len, (jshort*) data);
	//@mice: better yet, use "javap -s ClassName" to get all the signatures nicely printed to stdout :)
	jmethodID method = (*env)->GetStaticMethodID(env, cls, "onSampleReady", "([SI)V");
	if ( method != NULL)
		(*env)->CallStaticVoidMethod(env, cls, method, jBuff,len);

	//(*env)->ReleaseByteArrayElements(env, jBuff, data, JNI_ABORT );
	//(*jvm)->DetachCurrentThread(jvm);
...
JNIEXPORT void JNICALL Java_com_wordpress_ebc81_rtl_1ais_1android_core_RtlAisJava_open
	(JNIEnv * env, jclass class, jstring args, jint fd, jstring uspfs_path) {
		...
		rtl_ais_main(fd, n_uspfs_path, argc, argv);
