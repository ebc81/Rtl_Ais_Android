...
public class UsbPermissionHelper {
     ...
     */
    public static STATUS findDevice(final DeviceOpenActivity ctx, final boolean root) throws RtlStartException {
        ...
        if (!root && !force_root) {
            try {
                ...
                e.printStackTrace();
                if (e instanceof RtlStartException) throw (RtlStartException) e;
            }
        }
    ...
    public static void fixRootPermissions() throws RtlStartException {
        ...
        try { // Run Script
            ...
            throw new RtlStartException(err_info.root_required);
        } finally {
            if (osw != null) {
                try {
                    osw.close();
        ...
        if (proc.exitValue() != 0) {
            LogRtlAis.appendLine("Root refused to give permissions.");
            throw new RtlStartException(err_info.permission_denied);
        }
    }
